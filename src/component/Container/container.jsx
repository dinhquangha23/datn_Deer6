...
export default function Container() {
  ...
  useEffect(() => {
    if (localStorage.getItem("token") == null) {
      ...
      axios.get("getRamdomProduct").then((res) => setProducts(res.data.data));
    }
  }, []);

  useEffect(() => {
    idInterval.current = setInterval(() => {
      let currentProduct = slideReft.current.appendChild(
        slideReft.current.querySelectorAll(".product")[0]
      );
    }, 8000);
  ...
  function handleNext() {
    slideReft.current.prepend(
      slideReft.current.querySelectorAll(".product")[5]
    );
