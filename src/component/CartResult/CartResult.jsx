...
export default function CartResult({ data, keySerach, tongle }) {
  ...
  let list = getListKeyWord(keyWord, data?.name);
  // console.log(list)
  ...
  return (
    <div onClick={handelNavigate} className="result_container">
      ...
      <div className="result_container_content">
        <span className="result_content_title">
          {list[0]}
          <span className="height_light">{list[1]}</span>
          {list[2]}
        </span>
        <span className="result_content_price">
